...
原規劃為電腦與開發板直連 `192.168.50.x`。使用者的電腦沒有實體乙太網路埠（USB 埠已滿，無法接 USB 網卡），因此 2026-09-20 改為經由路由器：

```
開發板 ──網路線──> 路由器（192.168.1.1）──Wi-Fi──> 電腦
```

- 開發板：**先試 DHCP（逾時 15 秒），失敗才用固定 IP `192.168.1.200/24`**，閘道 `192.168.1.1`，MAC 由晶片唯一碼產生（本機管理位址，目前為 `02:1E:16:F4:22:E6`）。電腦端會自動找到開發板（先試記住的位址，再用廣播搜尋），見 `docs/dhcp_discovery.md`
- 電腦：Wi-Fi，DHCP 取得（目前 `192.168.1.247`）
- 串流：UDP，埠 5005，一個 UDP datagram 放一個完整封包
- **目的位址由電腦端先送封包告知**：電腦送出訂閱／保活封包，開發板記住來源位址與埠號後才開始送資料。原因有三：電腦 IP 由 DHCP 取得會變動；Windows 防火牆預設封鎖未經請求的輸入封包（電腦先送才會放行，不必新增防火牆規則）；規格要求 EEPROM 能「回應請求」。
...
- **M3 讀取時序**：設定更新率（基準 32 Hz subpage rate，挑戰 64 Hz），連續讀取 subpage。驗收：以計時標記量出單一 subpage 讀取時間，讀取在下一個 subpage 就緒前完成且留有餘裕（實測：讀取 17.5 ms／週期 31.7 ms＝55%，餘裕約 14 ms），連續 10 分鐘無遺漏、無順序錯誤、無 Melexis 資料檢查錯誤。
  - 使用者決定（2026-09-20）：原條件「低於週期的一半」在目前 I2C 硬體下做不到（見已知問題），改用上述條件，維持 32 Hz。
...
- ~~UDP 連線瞬間可能漏 1～2 個 subpage（ARP 等待）~~ **已解決**（2026-09-20）：網路移到核心 1，核心 0 專心讀感測器。見 `docs/dual_core.md`。
- **接偵錯器時必須先 `multicore_reset_core1()`**：J-Link 的 reset 只重置核心 0，核心 1 會繼續跑已被覆寫的舊韌體，導致 `multicore_launch_core1()` 的交握永遠不完成、開機卡住。`transport_udp.c` 已處理。日後任何用到核心 1 的韌體都要照做。
- **跨核心的 RTT log 需自行加鎖**：RTT 內建的鎖只擋同核心中斷。`log.h` 的 `LOG()` 已用硬體 spin lock 保護。
...
CMakeLists.txt  頂層建置檔（設定 Pico SDK 後 add_subdirectory(firmware)）
                ...
                燒錄檔在 build/images/（主韌體）與 build/images/tests/（測試），
                每支只輸出 .elf 與 .uf2，CMake 中間檔留在 build/firmware/
